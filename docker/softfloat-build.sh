...
for TARGET in $TARGETS; do
  ...
  if ([ $XGOOS == "." ] || [ $XGOOS == "linux" ]) && ([ $XGOARCH == "." ] || [ $XGOARCH == "mips" ]); then
    if [ "$GO_VERSION" -lt 180 ]; then
      ...
      CC=mips-linux-gnu-gcc-5 CFLAGS="-msoft-float" CXX=mips-linux-gnu-g++-5 CXXFLAGS="-msoft-float" HOST=mips-linux-gnu PREFIX=/usr/mips-linux-gnu $BUILD_DEPS /deps ${DEPS_ARGS[@]}
      export PKG_CONFIG_PATH=/usr/mips-linux-gnu/lib/pkgconfig

      CC=mips-linux-gnu-gcc-5 CXX=mips-linux-gnu-g++-5 GOOS=linux GOARCH=mips CGO_ENABLED=1 GOMIPS=softfloat CGO_CFLAGS="-msoft-float" CGO_CXXFLAGS="-msoft-float" go get $V $X "${T[@]}" --ldflags="$V $LD" -d ./$PACK
      CC=mips-linux-gnu-gcc-5 CXX=mips-linux-gnu-g++-5 GOOS=linux GOARCH=mips CGO_ENABLED=1 GOMIPS=softfloat CGO_CFLAGS="-msoft-float" CGO_CXXFLAGS="-msoft-float" go build $V $X "${T[@]}" --ldflags="$V $LD" $BM -o "/build/$NAME-linux-mips`extension linux`" ./$PACK
    fi
  fi
  if ([ $XGOOS == "." ] || [ $XGOOS == "linux" ]) && ([ $XGOARCH == "." ] || [ $XGOARCH == "mipsle" ]); then
    if [ "$GO_VERSION" -lt 180 ]; then
      ...
      CC=mipsel-linux-gnu-gcc-5 CFLAGS="-msoft-float" CXX=mipsel-linux-gnu-g++-5 CXXFLAGS="-msoft-float" HOST=mipsel-linux-gnu PREFIX=/usr/mipsel-linux-gnu $BUILD_DEPS /deps ${DEPS_ARGS[@]}
      export PKG_CONFIG_PATH=/usr/mipsle-linux-gnu/lib/pkgconfig

      CC=mipsel-linux-gnu-gcc-5 CXX=mipsel-linux-gnu-g++-5 GOOS=linux GOARCH=mipsle CGO_ENABLED=1 GOMIPS=softfloat CGO_CFLAGS="-msoft-float" CGO_CXXFLAGS="-msoft-float" go get $V $X "${T[@]}" --ldflags="$V $LD" -d ./$PACK
      CC=mipsel-linux-gnu-gcc-5 CXX=mipsel-linux-gnu-g++-5 GOOS=linux GOARCH=mipsle CGO_ENABLED=1 GOMIPS=softfloat CGO_CFLAGS="-msoft-float" CGO_CXXFLAGS="-msoft-float" go build $V $X "${T[@]}" --ldflags="$V $LD" $BM -o "/build/$NAME-linux-mipsle`extension linux`" ./$PACK
    fi
  fi
