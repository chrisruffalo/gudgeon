...
func newZoneSourceFromFile(zoneFile string) (Source, error) {
    ...
    for rr, hasNext := zp.Next(); true;  {
        ...
        if rr != nil && rr.Header() != nil {
            ...
            if _, found := source.records[name]; !found {
                source.records[name] = make(map[uint16]map[uint16][]dns.RR)
            }
            if _, found := source.records[name][rclass]; !found {
                source.records[name][rclass] = make(map[uint16][]dns.RR)   
            }
            if _, found := source.records[name][rclass][rrtype]; !found {
                source.records[name][rclass][rrtype] = make([]dns.RR, 0, 1)   
            }

            // add record to records for name and type
            source.records[name][rclass][rrtype] = append(source.records[name][rclass][rrtype], rr)
        }
